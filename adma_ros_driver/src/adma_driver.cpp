...
#include <adma_ros_driver_msgs/AdmaTruth.h>
#include <netdb.h>
#include <arpa/inet.h>
...
class ADMADriver {
        ...
        protected:
               void initializeUDP();
                ...
                void publishTruth(const adma_ros_driver_msgs::AdmaDataScaled& admaDataScaledMsg,
        ...
        private:
                ...
                bool _use_system_time = true;
                bool _publish_truth = true;
                bool _truth_log = false;
                double _truth_log_hz = 1.0;
                double _truth_log_period = 1.0;
                double _truth_rate_hz = 0.0;
                ros::Time _last_truth_pub;
...
ADMADriver::ADMADriver(ros::NodeHandle* n)
{
        ...
        if (!ros::param::get("adma_driver/use_system_time", _use_system_time)) {
                _use_system_time = true;
        }
        if (!ros::param::get("adma_driver/publish_truth", _publish_truth)) {
                _publish_truth = true;
        }
        if (!ros::param::get("adma_driver/truth_log", _truth_log)) {
                _truth_log = false;
        }
        if (!ros::param::get("adma_driver/truth_log_hz", _truth_log_hz)) {
                _truth_log_hz = 1.0;
        }
        if (_truth_log_hz <= 0.0) {
                _truth_log_hz = 1.0;
        }
        _truth_log_period = 1.0 / _truth_log_hz;
        if (!ros::param::get("adma_driver/truth_rate_hz", _truth_rate_hz)) {
                _truth_rate_hz = 0.0;
        }
        ...
        }else if (_protocol_version == "v3.3.4")
        {
                ...
                if (_publish_truth) {
                        _pubAdmaTruth = n->advertise<adma_ros_driver_msgs::AdmaTruth>("adma/truth", 10);
                }
        }
...
void ADMADriver::parseData(std::array<char, 856> recv_buf)
{
        ...
        float weektime;
        // Use system time for ROS headers by default.
        ros::Time system_time = ros::Time::now();
        ros::Time message_stamp = system_time;
        unsigned long long timestamp = static_cast<unsigned long long>(system_time.sec) * 1000ULL +
                                       static_cast<unsigned long long>(system_time.nsec) / 1000000ULL;
        // Offset between UNIX and GNSS (in ms), used if you switch back to ADMA time.
        ...
        if (_protocol_version == "v3.3.3")
        {
                ...
                if (!_use_system_time) {
                        ...
                        message_stamp = ros::Time(static_cast<double>(timestamp) / 1000.0);
                }
        ...
        }else if (_protocol_version == "v3.3.4")
        {
                ...
                admaDataScaledMsg.header.stamp = message_stamp;
                ...
                statusMsg.header.stamp = message_stamp;
                ...
                // Publish derived truth signals for convenience.
                publishTruth(admaDataScaledMsg, message_stamp);

                // publish ros standard messages
                message_fix.header.stamp = message_stamp;
                message_imu.header.stamp = message_stamp;
        ...
        rawDataMsg.header.stamp = message_stamp;
...
void ADMADriver::publishTruth(const adma_ros_driver_msgs::AdmaDataScaled& admaDataScaledMsg,
                              const ros::Time& stamp)
{
        if (!_publish_truth) {
                return;
        }
        if (_truth_rate_hz > 0.0) {
                double min_period = 1.0 / _truth_rate_hz;
                if (!_last_truth_pub.isZero() &&
                    (stamp - _last_truth_pub).toSec() < min_period) {
                        return;
                }
        }

        adma_ros_driver_msgs::AdmaTruth truthMsg;
        truthMsg.header.stamp = stamp;
        truthMsg.header.frame_id = _frame_id_adma;
        truthMsg.header.seq = _seq;
        truthMsg.actual_spd = std::sqrt(
                std::pow(admaDataScaledMsg.ins_vel_frame.x, 2) +
                std::pow(admaDataScaledMsg.ins_vel_frame.y, 2));
        truthMsg.yaw_rate = admaDataScaledMsg.rate_hor.z;
        truthMsg.long_accel = admaDataScaledMsg.acc_hor.x;
        truthMsg.lat_accel = admaDataScaledMsg.acc_hor.y;
        truthMsg.yaw_rate_sign = (truthMsg.yaw_rate < 0.0) ? 1 : 0;
        _pubAdmaTruth.publish(truthMsg);
        _last_truth_pub = stamp;

        if (_truth_log) {
                ROS_INFO_STREAM_THROTTLE(
                        _truth_log_period,
                        "adma/truth spd_mps=" << truthMsg.actual_spd
                        << " yaw_rate_dps=" << truthMsg.yaw_rate
                        << " lat_g=" << truthMsg.lat_accel
                        << " long_g=" << truthMsg.long_accel
                        << " yaw_sign=" << static_cast<int>(truthMsg.yaw_rate_sign));
        }
}
